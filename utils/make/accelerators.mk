...
$(ACC-baremetal): $(BAREMETAL_BIN) soft-build $(ESP_CFG_BUILD)/socmap.vhd
	...
	if [ `ls -1 $$ACC_PATH/sw/baremetal/*.c 2>/dev/null | wc -l ` -gt 0 ]; then \
		echo '   ' MAKE $@; \
		mkdir -p $$BUILD_PATH; \
		CROSS_COMPILE=$(CROSS_COMPILE_ELF) CPU_ARCH=$(CPU_ARCH) DRIVERS=$(DRV_BARE) DESIGN_PATH=$(DESIGN_PATH)/$(ESP_CFG_BUILD) BUILD_PATH=$$BUILD_PATH CACHELINE_WIDTH=$(CONFIG_CACHE_LINE_SIZE) EXTRA_CFLAGS='$(EXTRA_CFLAGS)' $(MAKE) -C  $$ACC_PATH/sw/baremetal; \
