...
ifneq ($(filter $(TECHLIB),$(FPGALIBS)),)
VSIMOPT += -L secureip_ver -L unisims_ver -L xpm
endif
...
VLIB = vlib
VCOM_93 = vcom -quiet -93 $(VCOMOPT)
VCOM_2008 = vcom -quiet -2008 $(VCOMOPT)
#VHDL2008_LIST := $(filter %/nocpackage.vhd\
	%/ESP_ASIC_TOP.vhd\
	%/chip_emu_top.vhd,\
	$(SIM_VHDL_PKGS) $(SIM_VHDL_SRCS))
VHDL93_PKGS   := $(filter-out $(VHDL2008_LIST),$(SIM_VHDL_PKGS))
VHDL93_SRCS   := $(filter-out $(VHDL2008_LIST),$(SIM_VHDL_SRCS))
...
modelsim/vsim.mk: modelsim/modelsim.ini $(RTL_CFG_BUILD)/check_all_srcs.old $(PKG_LIST) $(DESIGN_PATH)/Makefile
	@cd modelsim; \
...
ifneq ($(findstring profpga, $(BOARD)),)
	...
	for vhd in $(VHDL_PROFPGA); do \
		rtl=$(PROFPGA)/hdl/$$vhd; \
		echo $(SPACES)"$(VCOM_93) -work profpga $$rtl"; \
		$(VCOM_93) -work profpga $$rtl || exit; \
	done; \
	...
	for rtl in $(VHDL93_PKGS); do \
		echo $(SPACES)"$(VCOM_93) -work work $$rtl"; \
		$(VCOM_93) -work work $$rtl || exit; \
	done; \
	echo $(SPACES)"### Compile VHDL source files ###"; \
		for rtl in $(VHDL93_SRCS); do \
			echo $(SPACES)"$(VCOM_93) -work work $$rtl"; \
			$(VCOM_93) -work work $$rtl || exit; \
		done; \
	echo $(SPACES)"### Compile VHDL-2008 packages ###"; \
	for rtl in $(VHDL2008_LIST); do \
		echo $(SPACES)"$(VCOM_2008) -work work $$rtl"; \
		$(VCOM_2008) -work work $$rtl || exit; \
	done; \
