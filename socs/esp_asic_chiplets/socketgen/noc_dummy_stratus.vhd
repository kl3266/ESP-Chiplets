...
architecture rtl of noc_dummy_stratus is

  -- Device ID and revision numner
  constant vendorid      : vendor_t               := VENDOR_SLD;
  constant revision      : integer                := 0;
  constant exp_registers : integer range 0 to 1   := 0;
  constant devid         : devid_t                := SLD_DUMMY_STRATUS;
  constant tlb_entries   : integer                := 128;

  -- User defined registers
  -- bank(20): Numer of words to transfer from src to dst
  constant DUMMY_STRATUS_TOKENS_REG : integer range 0 to MAXREGNUM - 1 := 20;

  -- bank(21): Batches of tokens
  constant DUMMY_STRATUS_BATCH_REG : integer range 0 to MAXREGNUM - 1 := 21;

  -- bank(22): Source of P2P transfer
  constant DUMMY_STRATUS_SOURCE_REG : integer range 0 to MAXREGNUM - 1 := 22;

  -- bank(23): Number of multicast destinations
  constant DUMMY_STRATUS_NDESTS_REG : integer range 0 to MAXREGNUM - 1 := 23;
  ...
  constant available_reg_mask : std_logic_vector(0 to MAXREGNUM - 1) := (
    ...
    CHIP_YX_REG_3      => '1',
    SPANDEX_REG        => '1',
    DUMMY_STRATUS_TOKENS_REG            => '1',
    DUMMY_STRATUS_BATCH_REG             => '1',
    DUMMY_STRATUS_SOURCE_REG            => '1',
    DUMMY_STRATUS_NDESTS_REG            => '1',
    others             => '0');
...
begin
  ...
  dummy_stratus_rtl_i: dummy_stratus_rtl
    generic map (
      hls_conf => hls_conf
    )
    port map(
      conf_info_tokens           => bank(DUMMY_STRATUS_TOKENS_REG)(31 downto 0),
      conf_info_batch            => bank(DUMMY_STRATUS_BATCH_REG)(31 downto 0),
      conf_info_source           => bank(DUMMY_STRATUS_SOURCE_REG)(31 downto 0),
      conf_info_ndests           => bank(DUMMY_STRATUS_NDESTS_REG)(31 downto 0),
      clk                        => clk,
      acc_rst                    => acc_rst,
      conf_done                  => conf_done,
  ...
  l2_gen: if has_l2 /= 0 generate
    ...
    l2_acc_wrapper_1: l2_acc_wrapper
      generic map (
        ...
        cache_chip_y  => cache_chip_y,
        cache_chip_x  => cache_chip_x,
        cache_y       => cache_y,
        cache_x       => cache_x,
