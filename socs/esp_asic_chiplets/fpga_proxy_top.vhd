...
entity fpga_proxy_top is
  ...
  generic (
    ...
    JTAG_TRACE : integer range -1 to CFG_SIP_TILES_NUM - 1 := -1);
  port (
    ...
    -- Test interface
    tdi               : out   std_logic_vector(0 to CFG_SIP_TILES_NUM - 1);
    tdo               : in    std_logic_vector(0 to CFG_SIP_TILES_NUM - 1);
    tms               : out   std_logic;
    tclk              : out   std_logic;
    -- Ethernet signals
...
architecture rtl of fpga_proxy_top is
  ...
  function memswap (
    ...
    return integer is
  begin
    if CFG_NMEM_TILE = 4 then
      if n = 2 then
        return 2; -- KL return 3
      elsif n = 3 then
        return 3; -- KL return 2
      end if;
    end if;
    return n;
  end memswap;
  ...
  signal sending_packet : std_logic_vector(0 to CFG_NMEM_TILE - 1); --KL MUST CHANGE BACK TO -1
  signal receiving_packet : std_logic_vector(0 to CFG_NMEM_TILE - 1); --KL MUST CHANGE BACK TO -2

  signal target_y : local_yx;
  signal target_x : local_yx;
  signal target_chip_y : chip_yx;
  signal target_chip_x : chip_yx;
  ...
  -----------------------------------------------------------------------------
  -- JTAG
  signal tdi_int : std_logic_vector(0 to CFG_SIP_TILES_NUM - 1);
  signal tdo_int : std_logic_vector(0 to CFG_SIP_TILES_NUM - 1);
...
begin  -- architecture rtl
  ...
  gen_extra_sysclk : if SIMULATION = false generate
    gen_extra_i : for i in 1 to CFG_NMEM_TILE - 1 generate
      -- Reuse main_clk for auxiliary memory domains
      sys_clk(i) <= main_clk;
      sys_rst(i) <= not rstn;
    end generate gen_extra_i;
  end generate gen_extra_sysclk;
  ----------------------------------------------------------------------
...
--    end generate std_bus;
  ...
  begin
    ...
    for i in 0 to CFG_NMEM_TILE-1 loop
      if sending_packet(i) = '1' then
        ahbs_snd_wrreq(i) <= (not mux_ahbs_snd_empty) and (not ahbs_snd_full(i));
        ahbs_snd_data_in(i) <= mux_ahbs_snd_data_out;
        mux_ahbs_snd_rdreq  <= (not mux_ahbs_snd_empty) and (not ahbs_snd_full(i));
      end if;
    end loop;
...
--    end case;
  ...
  begin
    ...
    for i in 0 to CFG_NMEM_TILE-1 loop
      if receiving_packet(i) = '1' then
        mux_ahbs_rcv_wrreq   <= (not mux_ahbs_rcv_full) and (not ahbs_rcv_empty(i));
        mux_ahbs_rcv_data_in <= ahbs_rcv_data_out(i);
        ahbs_rcv_rdreq(i)    <= (not mux_ahbs_rcv_full) and (not ahbs_rcv_empty(i));
      end if;
    end loop;
...
--    end case;
  ...
  target_chip_x <= get_chip_destination_x(MISC_NOC_FLIT_SIZE, misc_noc_flit_pad & mux_ahbs_snd_data_out);
  target_chip_y <= get_chip_destination_y(MISC_NOC_FLIT_SIZE, misc_noc_flit_pad & mux_ahbs_snd_data_out);
  ...
  begin  -- process mux_state_gen
    if rstn = '0' then                   -- asynchronous reset (active low)
      ...
      -- Sender state
      if sending_packet = (sending_packet'range => '0') then
        -- Select target DDR
        if mux_ahbs_snd_empty = '0' then
          for i in 0 to CFG_NMEM_TILE - 1 loop
            if (target_chip_y = tile_mem_list(i).chip_y) and (target_chip_x = tile_mem_list(i).chip_x) and 
                (target_y = tile_mem_list(i).y) and (target_x = tile_mem_list(i).x) then
              sending_packet(i) <= '1';
            end if;
          end loop;  -- i
        end if;
        ...
        if (mux_ahbs_snd_empty = '0') and (mux_ahbs_snd_data_out(MISC_NOC_FLIT_SIZE - 2) = '1') and (ahbs_snd_full = (ahbs_snd_full'range => '0')) then
          sending_packet <= (others => '0');
        end if;
      end if;

      -- Receiver state
      if receiving_packet = (receiving_packet'range => '0') then
        -- Select source DDR
        for i in 0 to CFG_NMEM_TILE - 1 loop
  ...
  ahbslv2noc_1 : ahbslv2noc
    generic map (
      ...
      slv_y                 => tile_y(io_tile_id(0)),
      slv_x                 => tile_x(io_tile_id(0)),
      slv_chip_y            => chip_y(io_tile_id(0)),
      slv_chip_x            => chip_x(io_tile_id(0)),
      retarget_for_dma      => 1,
      dma_length            => CFG_DLINE)
    port map (
      rst                        => rstn,
      clk                        => main_clk,
      local_y                    => tile_y(io_tile_id(0)),
      local_x                    => tile_x(io_tile_id(0)),
      local_chip_y               => chip_y(io_tile_id(0)),
      local_chip_x               => chip_x(io_tile_id(0)),
      ahbsi                      => ahbsi,
      ahbso                      => ahbso_edcl,
      dma_selected               => '0',
  ...
  ahbctrl_edcl_1 : ahbctrl                        -- AHB arbiter/multiplexer
    generic map (defmast => CFG_DEFMST, split => CFG_SPLIT,
                 rrobin  => CFG_RROBIN, ioaddr => CFG_AHBIO, fpnpen => CFG_FPNPEN,
                 nahbm   => 3, nahbs => NAHBSLV,
                 cfgmask => 0)
    port map (rstn, main_clk, ahbmi, ahbmo, ahbsi, ahbso);

  -- Unused bus ports
  unused_ahbm_eth_gen: for j in 3 to NAHBMST - 1 generate -- KL changed from NAHBSLV to NAHBMST
    ahbmo(j) <= ahbm_none;
  ...
  tdi_pad_gen : for i in 0 to CFG_SIP_TILES_NUM -1 generate
  ...
  tdo_pad_gen : for i in 0 to CFG_SIP_TILES_NUM -1 generate
...
--  gen_jtag : if JTAG_TRACE /= -1 generate
...
--  gen_no_jtag : if JTAG_TRACE = -1 generate
--    ahbso_apb1 <= ahbs_none;
--  end generate gen_no_jtag;
  ...
  jtag_driver_gen: if JTAG_TRACE /= -1 generate
    ...
    jtag_gen_sim: if SIMULATION = true  generate
      ...
      tdi_gen: for i in 0 to CFG_SIP_TILES_NUM - 1 generate
...
--  iolink_no_sim_gen: if SIMULATION = false generate
    ahbslv2iolink_i : ahbslv2iolink --KL not sure about word_bitwidth
      generic map (
        hindex => 2,
