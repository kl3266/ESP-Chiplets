...
-----------------------------------------------------------------------------
--  Testbench for ESP on proFPGA xcvu19p chiplets with DDR4 and Ethernet
------------------------------------------------------------------------------
...
--  signal c7_sys_clk_n : std_ulogic := '1';
  ...
  signal b0_b1_cable_clk_n  : std_logic := '1';
  signal b1_b0_cable_clk_p  : std_logic := '0';
  signal b1_b0_cable_clk_n  : std_logic := '1';
  signal b0_b2_cable_clk_p  : std_logic := '0';
  signal b0_b2_cable_clk_n  : std_logic := '1';
  signal b2_b0_cable_clk_p  : std_logic := '0';
  signal b2_b0_cable_clk_n  : std_logic := '1';
  signal b1_b3_cable_clk_p  : std_logic := '0';
  signal b1_b3_cable_clk_n  : std_logic := '1';
  signal b3_b1_cable_clk_p  : std_logic := '0';
  signal b3_b1_cable_clk_n  : std_logic := '1';
  signal b2_b3_cable_clk_p  : std_logic := '0';
  signal b2_b3_cable_clk_n  : std_logic := '1';
  signal b3_b2_cable_clk_p  : std_logic := '0';
  signal b3_b2_cable_clk_n  : std_logic := '1';

  signal b0_b1_cable_data   : std_logic_vector(135 downto 0) := (others => 'Z'); -- first 68b send; second 68b rcv;
  signal b0_b2_cable_data   : std_logic_vector(135 downto 0) := (others => 'Z'); -- first 68b send; second 68b rcv;
  signal b1_b3_cable_data   : std_logic_vector(135 downto 0) := (others => 'Z'); -- first 68b send; second 68b rcv;
  signal b2_b3_cable_data   : std_logic_vector(135 downto 0) := (others => 'Z'); -- first 68b send; second 68b rcv;
...
--  signal c7_diagnostic_led : std_ulogic; 
  ...
  signal open_ddr4_dm_dbi_n : std_logic_vector(8 downto 0);
  signal open_ddr4_dq       : std_logic_vector(71 downto 0);
  signal open_ddr4_dqs_c    : std_logic_vector(8 downto 0);
  signal open_ddr4_dqs_t    : std_logic_vector(8 downto 0);
  signal open_emdio         : std_logic;
  ...
  component top
    ...
    port (
      ...
      -- Main ESP clock
      esp_clk_p         : in    std_ulogic;  -- 50 MHz clock
...
begin
  ...
  open_ddr4_dm_dbi_n <= (others => 'Z');
  open_ddr4_dq       <= (others => 'Z');
  open_ddr4_dqs_c    <= (others => 'Z');
  open_ddr4_dqs_t    <= (others => 'Z');
  open_emdio         <= 'Z';
...
--  c7_ddr4_dqs_t    <= (others => 'Z');
  ...
  top_gen : for i in 0 to 3 generate
    board0_gen : if i = 0 generate
      top_0 : top
        generic map (
        ...
        port map (
          ...
          c0_cable_clk_p    => b0_b1_cable_clk_p,
          c0_cable_clk_n    => b0_b1_cable_clk_n,
          c0_cable_clk_p_rcv  => b1_b0_cable_clk_p,
          c0_cable_clk_n_rcv  => b1_b0_cable_clk_n,
          c0_cable_io_data  => b0_b1_cable_data,

          c1_cable_clk_p    => b0_b2_cable_clk_p,
          c1_cable_clk_n    => b0_b2_cable_clk_n,
          c1_cable_clk_p_rcv  => b2_b0_cable_clk_p,
          c1_cable_clk_n_rcv  => b2_b0_cable_clk_n,
          c1_cable_io_data  => b0_b2_cable_data,
    ...
    --      tft_npd           => tft_npd,
        ...
        end generate board0_gen;
    board1_gen : if i = 1 generate
      top_1 : top_satellite
        generic map (
          SIMULATION => SIMULATION,
          BOARD_NUM  => i
        ...
        port map (
          ...
          -- IO Cable (blue-ribbon)
          c0_cable_clk_p    => b1_b0_cable_clk_p,
          c0_cable_clk_n    => b1_b0_cable_clk_n,
          c0_cable_clk_p_rcv  => b0_b1_cable_clk_p,
          c0_cable_clk_n_rcv  => b0_b1_cable_clk_n,
          c0_cable_io_data  => b0_b1_cable_data,

          c1_cable_clk_p    => b1_b3_cable_clk_p,
          c1_cable_clk_n    => b1_b3_cable_clk_n,
          c1_cable_clk_p_rcv  => b3_b1_cable_clk_p,
          c1_cable_clk_n_rcv  => b3_b1_cable_clk_n,
          c1_cable_io_data  => b1_b3_cable_data,
          -- DDR4
          ...
          c0_calib_complete => c1_calib_complete,
          c0_diagnostic_led => c1_diagnostic_led,
    --       c1_sys_clk_p      => '0',
    --       c1_sys_clk_n      => '1',
    --       c1_ddr4_act_n     => open,
    --       c1_ddr4_adr       => open,
    ...
    --       c1_ddr4_ck_t      => open,
    --       c1_ddr4_ck_c      => open,
    --       c1_ddr4_reset_n   => open,
    --       c1_ddr4_dm_dbi_n  => open_ddr4_dm_dbi_n,
    --       c1_ddr4_dq        => open_ddr4_dq,
    --       c1_ddr4_dqs_c     => open_ddr4_dqs_c,
    --       c1_ddr4_dqs_t     => open_ddr4_dqs_t,
    --       c1_calib_complete => open,
    --       c1_diagnostic_led => open,
    --       c2_sys_clk_p      => '0',
    --       c2_sys_clk_n      => '1',
    ...
    --       c2_ddr4_ck_t      => open,
    --       c2_ddr4_ck_c      => open,
    --       c2_ddr4_reset_n   => open,
    --       c2_ddr4_dm_dbi_n  => open_ddr4_dm_dbi_n,
    --       c2_ddr4_dq        => open_ddr4_dq,
    --       c2_ddr4_dqs_c     => open_ddr4_dqs_c,
    --       c2_ddr4_dqs_t     => open_ddr4_dqs_t,
    --       c2_calib_complete => open,
    --       c2_diagnostic_led => open,
    --       c3_sys_clk_p      => '0',
    --       c3_sys_clk_n      => '1',
    ...
    --       c3_ddr4_ck_t      => open,
    --       c3_ddr4_ck_c      => open,
    --       c3_ddr4_reset_n   => open,
    --       c3_ddr4_dm_dbi_n  => open_ddr4_dm_dbi_n,
    --       c3_ddr4_dq        => open_ddr4_dq,
    --       c3_ddr4_dqs_c     => open_ddr4_dqs_c,
    --       c3_ddr4_dqs_t     => open_ddr4_dqs_t,
    --       c3_calib_complete => open,
    --       c3_diagnostic_led => open,
    --       c4_sys_clk_p      => '0',
    --       c4_sys_clk_n      => '1',
    ...
    --       c4_ddr4_ck_t      => open,
    --       c4_ddr4_ck_c      => open,
    --       c4_ddr4_reset_n   => open,
    --       c4_ddr4_dm_dbi_n  => open_ddr4_dm_dbi_n,
    --       c4_ddr4_dq        => open_ddr4_dq,
    --       c4_ddr4_dqs_c     => open_ddr4_dqs_c,
    --       c4_ddr4_dqs_t     => open_ddr4_dqs_t,
    --       c4_calib_complete => open,
    --       c4_diagnostic_led => open,
    --       c5_sys_clk_p      => '0',
    --       c5_sys_clk_n      => '1',
    ...
    --       c5_ddr4_ck_t      => open,
    --       c5_ddr4_ck_c      => open,
    --       c5_ddr4_reset_n   => open,
    --       c5_ddr4_dm_dbi_n  => open_ddr4_dm_dbi_n,
    --       c5_ddr4_dq        => open_ddr4_dq,
    --       c5_ddr4_dqs_c     => open_ddr4_dqs_c,
    --       c5_ddr4_dqs_t     => open_ddr4_dqs_t,
    --       c5_calib_complete => open,
    --       c5_diagnostic_led => open,
    --       c6_sys_clk_p      => '0',
    --       c6_sys_clk_n      => '1',
    ...
    --       c6_ddr4_ck_t      => open,
    --       c6_ddr4_ck_c      => open,
    --       c6_ddr4_reset_n   => open,
    --       c6_ddr4_dm_dbi_n  => open_ddr4_dm_dbi_n,
    --       c6_ddr4_dq        => open_ddr4_dq,
    --       c6_ddr4_dqs_c     => open_ddr4_dqs_c,
    --       c6_ddr4_dqs_t     => open_ddr4_dqs_t,
    --       c6_calib_complete => open,
    --       c6_diagnostic_led => open,
    -- --      c7_sys_clk_p      => c7_sys_clk_p,
    -- --      c7_sys_clk_n      => c7_sys_clk_n,
    ...
    -- --      tft_npd           => tft_npd,
        ...
        end generate board1_gen;
    board2_gen : if i = 2 generate
      top_2 : top_satellite
        generic map (
          SIMULATION => SIMULATION,
          BOARD_NUM  => i
        ...
        port map (
          ...
          c0_cable_clk_n    => b2_b3_cable_clk_n,
          c0_cable_clk_p_rcv  => b3_b2_cable_clk_p,
          c0_cable_clk_n_rcv  => b3_b2_cable_clk_n,
          c0_cable_io_data  => b2_b3_cable_data,
          c1_cable_clk_p    => b2_b0_cable_clk_p,
          c1_cable_clk_n    => b2_b0_cable_clk_n,
          c1_cable_clk_p_rcv  => b0_b2_cable_clk_p,
          c1_cable_clk_n_rcv  => b0_b2_cable_clk_n,
          c1_cable_io_data  => b0_b2_cable_data,
          -- DDR4
          ...
          c0_calib_complete => c2_calib_complete,
          c0_diagnostic_led => c2_diagnostic_led,
    --       c1_sys_clk_p      => '0',
    --       c1_sys_clk_n      => '1',
    --       c1_ddr4_act_n     => open,
    --       c1_ddr4_adr       => open,
    ...
    -- --      tft_npd           => tft_npd,
        ...
        end generate board2_gen;
    board3_gen : if i = 3 generate
      top_3 : top_satellite
        generic map (
          SIMULATION => SIMULATION,
          BOARD_NUM  => i
        ...
        port map (
          ...
          c0_cable_clk_n    => b3_b2_cable_clk_n,
          c0_cable_clk_p_rcv  => b2_b3_cable_clk_p,
          c0_cable_clk_n_rcv  => b2_b3_cable_clk_n,
          c0_cable_io_data  => b2_b3_cable_data,
          c1_cable_clk_p    => b3_b1_cable_clk_p,
          c1_cable_clk_n    => b3_b1_cable_clk_n,
          c1_cable_clk_p_rcv  => b1_b3_cable_clk_p,
          c1_cable_clk_n_rcv  => b1_b3_cable_clk_n,
          c1_cable_io_data  => b1_b3_cable_data,
          -- DDR4
          ...
          c0_calib_complete => c3_calib_complete,
          c0_diagnostic_led => c3_diagnostic_led,
    --       c1_sys_clk_p      => '0',
    --       c1_sys_clk_n      => '1',
    --       c1_ddr4_act_n     => open,
    --       c1_ddr4_adr       => open,
