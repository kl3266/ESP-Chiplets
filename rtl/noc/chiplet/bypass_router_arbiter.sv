...
//  logic [3:0] ptr;
//  logic [3:0] mask;
//  logic [3:0] req_masked, req_gated;
//  logic [3:0] grant_masked, grant_raw;
//
//  assign req_gated = grant_locked ? '0 : request;
//  assign grant_valid = |req_gated;
//
//  assign mask[0] = 1'b0;
//  assign mask[1] = ptr[0];
//  assign mask[2] = ptr[0] | ptr[1];
//  assign mask[3] = ptr[0] | ptr[1] | ptr[2];
//
...
//    grant_masked = '0;
//    priority case (1'b1)
//      req_masked[0] : grant_masked = 4'b0001;
//      req_masked[1] : grant_masked = 4'b0010;
//      req_masked[2] : grant_masked = 4'b0100;
//      req_masked[3] : grant_masked = 4'b1000;
//    endcase
//  end
//
//  always_comb begin
//    grant_raw = '0;
//    priority case (1'b1)
//      req_gated[0] : grant_raw = 4'b0001;
//      req_gated[1] : grant_raw = 4'b0010;
//      req_gated[2] : grant_raw = 4'b0100;
//      req_gated[3] : grant_raw = 4'b1000;
//    endcase
//  end
//
//  assign grant_valid = |req_gated;
...
//    end else if (forwarding_tail) begin
//      grant_locked <= 1'b0;
//    end else if (forwarding_head && grant_valid) begin
//      grant_locked <= 1'b1;
...
module bypass_router_arbiter_rtpe
  ...
  logic grant_locked;
  logic [1:0] ptr;
  logic [2:0] req_masked, req_internal;
  logic [2:0] mask;
  logic [2:0] grant_masked, grant_raw;

  assign req_internal = grant_locked ? 3'b0 : request;
  assign mask = 3'b111 << ptr;
