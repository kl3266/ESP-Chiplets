...
(
  ...
  localparam int unsigned LEVELS = calc_reduce_levels(N, 1);
  ...
  generate
    ...
    if (LEVELS == 0) begin : gen_passthrough
      ...
      for (genvar level = 1; level <= LEVELS; level++) begin : gen_levels
        localparam int unsigned CUR_COUNT = level_count(N, level - 1, 1);
        localparam int unsigned NEXT_COUNT = level_count(N, level, 1);
