...
-------------------------------------------------------------------------------
  ...
  constant CHIP_YX_WIDTH       : natural := 3;
  constant MSG_TYPE_WIDTH      : natural := 5;
  constant RESERVED_WIDTH      : natural := 8;
  constant RESERVED_WIDTH_MISC : natural := 6;
  constant NEXT_ROUTING_WIDTH  : natural := 5;
  constant PLANE_WIDTH         : natural := 3;
  constant TILE_WIDTH          : natural := 4;
  constant ROUTING_LOCK_WIDTH  : natural := 1;
  ...
  constant MISC_NOC_FLIT_SIZE  : natural := PREAMBLE_WIDTH + 64;
  ...
  subtype chip_yx is std_logic_vector(CHIP_YX_WIDTH-1 downto 0);
  ...
  subtype tile_t is std_logic_vector(3 downto 0);
  subtype plane_t is std_logic_vector(2 downto 0);

  type ucie_trx is array (3 downto 0) of std_logic_vector(63 downto 0);
  type ucie_trxrd is array (3 downto 0) of std_logic_vector(3 downto 0);
  type ucie_apb_prdata is array (3 downto 0) of std_logic_vector(31 downto 0);
  ...
  type chip_dest_arr is array (natural range <>) of chip_yx;
...
-- -- Message type encoding
  ...
  type chip_yx_vec is array (natural range <>) of std_logic_vector(CHIP_YX_WIDTH - 1 downto 0);
  ...
  function get_chip_origin_y (
    constant flit_sz : integer;
    flit : max_noc_flit_type)
    return chip_yx;

  function get_chip_origin_x (
    constant flit_sz : integer;
    flit : max_noc_flit_type)
    return chip_yx;

  function get_chip_destination_y (
    constant flit_sz : integer;
    flit : max_noc_flit_type)
    return chip_yx;

  function get_chip_destination_x (
    constant flit_sz : integer;
    flit : max_noc_flit_type)
    return chip_yx;
  ...
  function get_chip_origin_y_misc (
    flit : misc_noc_flit_type)
    return chip_yx;

  function get_chip_origin_x_misc (
    flit : misc_noc_flit_type)
    return chip_yx;

  function get_chip_destination_y_misc (
    flit : misc_noc_flit_type)
    return chip_yx;

  function get_chip_destination_x_misc (
    flit : misc_noc_flit_type)
    return chip_yx;
  ...
  function set_router_ports (
    ...
    constant CFG_CHIP_XLEN : integer;
    constant CFG_CHIP_YLEN : integer;
    constant CFG_XLEN : integer;
    constant CFG_YLEN : integer;
    constant local_x  : local_yx;
    constant local_y  : local_yx;
    constant local_chip_x : chip_yx;
    constant local_chip_y : chip_yx)
    return ports_vec;
...
package body nocpackage is
  ...
  function get_chip_origin_y (
    ...
    flit : max_noc_flit_type)
    return chip_yx is
    variable ret : chip_yx;
  begin  -- get_chip_origin_y
    ret := (others => '0');
    ret := flit(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 1 downto flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - CHIP_YX_WIDTH);
    return ret;
  end get_chip_origin_y;
  ...
  function get_chip_origin_x (
    ...
    return chip_yx is
    variable ret : chip_yx;
  begin  -- get_chip_origin_x
    ret := (others => '0');
    ret := flit(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - CHIP_YX_WIDTH - 1 downto flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 2*CHIP_YX_WIDTH);
    return ret;
  end get_chip_origin_x;
  ...
  function get_chip_destination_y (
    ...
    return chip_yx is
    variable ret : chip_yx;
  begin  -- get_chip_destination_y
    ret := (others => '0');
    ret := flit(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 2*CHIP_YX_WIDTH - 1 downto flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 3*CHIP_YX_WIDTH);
    return ret;
  end get_chip_destination_y;
  ...
  function get_chip_destination_x (
    ...
    return chip_yx is
    variable ret : chip_yx;
  begin  -- get_chip_destination_x
    ret := (others => '0');
    ret := flit(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 3*CHIP_YX_WIDTH - 1 downto flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 4*CHIP_YX_WIDTH);
    return ret;
  end get_chip_destination_x;
  ...
  begin
    ret := flit(NEXT_ROUTING_WIDTH + RESERVED_WIDTH - 1 downto
                NEXT_ROUTING_WIDTH);
    return ret;
  ...
  begin
    ret := flit(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 4*CHIP_YX_WIDTH - 1);
    return ret;
  ...
  begin  -- get_origin_y
    ret := (others => '0');
    ret := flit(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 1 downto MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - CHIP_YX_WIDTH);
    return ret;
  end get_chip_origin_y_misc;
  ...
  begin  -- get_origin_x
    ret := (others => '0');
    ret := flit(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - CHIP_YX_WIDTH - 1 downto MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 2*CHIP_YX_WIDTH);
    return ret;
  end get_chip_origin_x_misc;
  ...
  begin  -- get_chip_destination_y
    ret := (others => '0');
    ret := flit(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 2*CHIP_YX_WIDTH - 1 downto MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 3*CHIP_YX_WIDTH);
    return ret;
  end get_chip_destination_y_misc;
  ...
  begin  -- get_chip_destination_x
    ret := (others => '0');
    ret := flit(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 3*CHIP_YX_WIDTH - 1 downto MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 4*CHIP_YX_WIDTH);
    return ret;
  end get_chip_destination_x_misc;
  ...
  begin
    ret := flit(NEXT_ROUTING_WIDTH + RESERVED_WIDTH_MISC - 1 downto
                NEXT_ROUTING_WIDTH);
    return ret;
  ...
  function get_unused_msb_field_misc (
    ...
    variable ret : std_ulogic;
  begin
    ret := flit(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 4*CHIP_YX_WIDTH - 1);
    return ret;
  ...
  function create_header (
    ...
    variable go_left_chip, go_right_chip, go_up_chip, go_down_chip : std_logic_vector(NEXT_ROUTING_WIDTH - 1 downto 0);
    variable next_routing, next_routing_chip   : std_logic_vector(NEXT_ROUTING_WIDTH - 1 downto 0);
  ...
  begin  -- create_header
    ...
    header(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - 1 downto
           flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH) := "000";
    ...
    header(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - CHIP_YX_WIDTH - 1 downto
           flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 2*CHIP_YX_WIDTH) := local_chip_x;
    header(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 2*CHIP_YX_WIDTH - 1 downto
           flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 3*CHIP_YX_WIDTH) := remote_chip_y;
    header(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 3*CHIP_YX_WIDTH - 1 downto
           flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 4*CHIP_YX_WIDTH) := remote_chip_x; 
    header(NEXT_ROUTING_WIDTH + RESERVED_WIDTH - 1 downto
           NEXT_ROUTING_WIDTH) := reserved;

    if local_chip_x < remote_chip_x then
      go_right_chip := "01000";
    else
      go_right_chip := "10111";
    end if;
    ...
    if local_chip_y < remote_chip_y then
      next_routing_chip := "01110" and go_left_chip and go_right_chip;
    else
      next_routing_chip := "01101" and go_left_chip and go_right_chip;
    end if;
    ...
    if local_y < remote_y then
      next_routing := "01110" and go_left and go_right;
    else
      next_routing := "01101" and go_left and go_right;
    end if;
    ...
    if local_chip_y = remote_chip_y and local_chip_x = remote_chip_x then
      routing_lock := '0';
      header(NEXT_ROUTING_WIDTH - 1 downto 0) := next_routing;
    else
      routing_lock := '1';
      header(NEXT_ROUTING_WIDTH - 1 downto 0) := next_routing_chip;
    end if;
    header(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - 1) := routing_lock;
  ...
  begin  -- create_header
    ...
    header(flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 3*CHIP_YX_WIDTH - 1 downto
           flit_sz - PREAMBLE_WIDTH - 4*YX_WIDTH - MSG_TYPE_WIDTH - TILE_WIDTH - PLANE_WIDTH - ROUTING_LOCK_WIDTH - 4*CHIP_YX_WIDTH) := remote_chip_x; 
    header(NEXT_ROUTING_WIDTH + RESERVED_WIDTH_MISC - 1 downto
           NEXT_ROUTING_WIDTH) := reserved;
...
--        narrow_flit(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 1 downto 0);
      ret(31 downto 0) := narrow_flit(31 downto 0);
    end if;
...
--        large_flit(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 1 downto 0);
      ret(31 downto 0) := large_flit(31 downto 0);
    end if;
...
-- This function will go to top and OUTER_PORTS will be passed through parameter
  ...
  begin
    ...
    ports := (others => '1');
    -- north ports removed in top tiles
    if local_y = conv_std_logic_vector(0, YX_WIDTH) and local_chip_y = conv_std_logic_vector(0, CHIP_YX_WIDTH) then
      ports(0) := '0';
    end if;
    -- west ports removed in left edge tiles
    if local_x = conv_std_logic_vector(0, YX_WIDTH) and local_chip_x = conv_std_logic_vector(0, CHIP_YX_WIDTH) then
      ports(2) := '0';
    end if;
--    if is_fpga(TECH) /= 0 then
    ...
    if (to_integer(unsigned(local_y))) = CFG_YLEN-1 and (to_integer(unsigned(local_chip_y))) = CFG_CHIP_YLEN-1 then
      ports(1) := '0';
    end if;
    -- east ports removed in right edge tiles
    if (to_integer(unsigned(local_x))) = CFG_XLEN-1 and (to_integer(unsigned(local_chip_x))) = CFG_CHIP_XLEN-1 then
      ports(3) := '0';
    end if;
