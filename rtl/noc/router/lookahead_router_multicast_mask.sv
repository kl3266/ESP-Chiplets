...
module lookahead_router_multicast_mask #(
    ...
    typedef struct packed {
        ...
        noc::routing_lock_t routing_lock; // 0 - enable on-chip routing; 1 - disable on-chip routing
        ...
        logic [ReservedWidth+$bits(noc::chip_xy_t)-1:0] reserved;
      } packet_info_t;
...
//            assign mcast_input_initial[g_i] = |(bp_frr[g_i] & (bp_frr[g_i] - 1));
            assign mcast_input_initial[g_i] = (bp_frr[g_i][0] & (|bp_frr[g_i][4:1])) |
                                              (bp_frr[g_i][1] & (|bp_frr[g_i][4:2])) |
                                              (bp_frr[g_i][2] & (|bp_frr[g_i][4:3])) |
                                              (bp_frr[g_i][3] & bp_frr[g_i][4]);
            ...
            logic [4:0] rd_fifo_vector;
            assign rd_fifo_vector = { enhanc_routing_configuration[4][g_i],
                                      enhanc_routing_configuration[3][g_i],
    ...
    always_comb begin
        logic [4:0] req [4:0];
        logic [4:0] col_vec, conflict_mask, port_blocked;

        for (int i = 0; i < 5; i++) begin
            req[i] = final_routing_request[i] & {5{mcast_input_initial[i]}};
            for (int j = 0; j < 5; j++) begin
        ...
        for (int i = 0; i < 5; i++) begin  //i = output
            col_vec = {req[4][i], req[3][i], req[2][i], req[1][i], req[0][i]};
            //col_vec = {case_b[4][i], case_b[3][i], case_b[2][i], case_b[1][i], case_b[0][i]}; // Above line --> better for area and timing. This one improves performance by ~10%
            //if ((col_vec & (col_vec - 5'b1)) != '0) begin
            conflict_mask[i] = (col_vec[0] & (|col_vec[4:1])) |
...
//            );
            ...
            assign rd_fifo[g_i] = {5{no_backpressure_mcast[g_i] && forwarding_in_progress[g_i]}} & enhanc_routing_configuration[g_i];
...
//                end
//            end
            if (g_i < 4) begin : gen_link_output
              noc::xy_t selected_destination;
              noc::xy_t selected_destination_mask;
              noc::routing_lock_t selected_routing_lock;
              noc::mcast_routing_lock_t selected_mcast_routing_lock;
              noc::direction_t selected_routing;
              noc::mcast_t selected_mcast_type;
              ...
              always_comb begin : crossbar_onehot_case
                ...
                selected_routing_lock = '0;
                selected_mcast_routing_lock = '0;
                selected_routing = '0;
                selected_mcast_type = '0;
                selected_uni_or_mcast = '0;
  
                unique0 case (1'b1)
                  enhanc_routing_configuration[g_i][0]: begin
                    fifo_head_routing[g_i]    = fifo_head[0];
                    out_unvalid_flit[g_i]     = in_unvalid_flit[0];
                    selected_destination      = fifo_head[0].header.info.destination;
                    selected_destination_mask = fifo_head[0].header.info.destination_mask;
                    selected_routing_lock     = fifo_head[0].header.info.routing_lock;
                    selected_mcast_routing_lock = fifo_head[0].header.info.mcast_routing_lock;
                    selected_routing          = fifo_head[0].header.routing;
                    selected_mcast_type       = fifo_head[0].header.info.mcast_type;
                  ...
                  enhanc_routing_configuration[g_i][1]: begin
                    ...
                    selected_destination      = fifo_head[1].header.info.destination;
                    selected_destination_mask = fifo_head[1].header.info.destination_mask;
                    selected_routing_lock     = fifo_head[1].header.info.routing_lock;
                    selected_mcast_routing_lock = fifo_head[1].header.info.mcast_routing_lock;
                    selected_routing          = fifo_head[1].header.routing;
                    selected_mcast_type       = fifo_head[1].header.info.mcast_type;
                  ...
                  enhanc_routing_configuration[g_i][2]: begin
                    ...
                    selected_destination      = fifo_head[2].header.info.destination;
                    selected_destination_mask = fifo_head[2].header.info.destination_mask;
                    selected_routing_lock     = fifo_head[2].header.info.routing_lock;
                    selected_mcast_routing_lock = fifo_head[2].header.info.mcast_routing_lock;
                    selected_routing          = fifo_head[2].header.routing;
                    selected_mcast_type       = fifo_head[2].header.info.mcast_type;
                  ...
                  enhanc_routing_configuration[g_i][3]: begin
                    ...
                    selected_destination      = fifo_head[3].header.info.destination;
                    selected_destination_mask = fifo_head[3].header.info.destination_mask;
                    selected_routing_lock     = fifo_head[3].header.info.routing_lock;
                    selected_mcast_routing_lock = fifo_head[3].header.info.mcast_routing_lock;
                    selected_routing          = fifo_head[3].header.routing;
                    selected_mcast_type       = fifo_head[3].header.info.mcast_type;
                  ...
                  enhanc_routing_configuration[g_i][4]: begin
                    ...
                    selected_destination      = fifo_head[4].header.info.destination;
                    selected_destination_mask = fifo_head[4].header.info.destination_mask;
                    selected_routing_lock     = fifo_head[4].header.info.routing_lock;
                    selected_mcast_routing_lock = fifo_head[4].header.info.mcast_routing_lock;
                    selected_routing          = fifo_head[4].header.routing;
                    selected_mcast_type       = fifo_head[4].header.info.mcast_type;
                    selected_uni_or_mcast    = fifo_head[4].header.info.uni_or_mcast;
                  end
                endcase
              end

              // convert unicast routing lock to mcast routing lock
              noc::mcast_routing_lock_t mcast_rl_convert;
              assign mcast_rl_convert = (selected_routing_lock && (!selected_uni_or_mcast)) ? selected_routing[3:0] : selected_mcast_routing_lock;  // unicast and routing lock on

              localparam logic [1:0] PORT_ID_L = g_i[1:0];
              noc::direction_t next_hop_routing;
              noc::mcast_routing_lock_t next_mcast_routing_lock;

              lookahead_routing_multicast_mask #(
                  .W($bits(position.x)),
                  .PORT_ID(PORT_ID_L)
              ) lookahead_routing_multicast_mask_i (
                  .clk,
                  .position,
                  .destination(selected_destination),
                  .destination_mask(selected_destination_mask),
                  .mcast_routing_lock(mcast_rl_convert),
                  .mcast_type(selected_mcast_type),  // 1 - brcp, disable y traversals.
                //   .uni_or_mcast(selected_uni_or_mcast),  // 1 - multicast; 0 - unicast
                  .routing_lock(selected_routing_lock),
                  .next_mcast_routing_lock(next_mcast_routing_lock),
              ...
              always_comb begin
                ...
                if (insert_lookahead_routing[g_i]) begin
                  data_out_crossbar[g_i].header.info.mcast_routing_lock = next_mcast_routing_lock;
                  data_out_crossbar[g_i].header.routing = next_hop_routing;
                end
              end
            end else begin : gen_local_output
              always_comb begin : crossbar_onehot_case
                fifo_head_routing[g_i] = '0;
                out_unvalid_flit[g_i]  = 1'b1;
              ...
              assign data_out_crossbar[g_i] = fifo_head_routing[g_i];
            end
