...
-------------------------------------------------------------------------------
  rcv_fsm : process (rcv_reg,
    ...
    variable orig_chip_y, orig_chip_x : chip_yx;
    variable tile_id        : integer range 0 to CFG_SIP_TILES_NUM;
  ...
  begin  -- process rcv_fsm
    ...
    orig_chip_y := noc5_output_port(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4 * YX_WIDTH - MSG_TYPE_WIDTH - 1 downto MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH -4 * YX_WIDTH - MSG_TYPE_WIDTH - CHIP_YX_WIDTH);
    orig_chip_x := noc5_output_port(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4 * YX_WIDTH - MSG_TYPE_WIDTH - CHIP_YX_WIDTH - 1 downto MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 4 * YX_WIDTH - MSG_TYPE_WIDTH - 2 * CHIP_YX_WIDTH);
    if unsigned(orig_x) >= 0 and unsigned(orig_x) < CFG_XLEN(chiplet_index) and
      unsigned(orig_y) >= 0 and unsigned(orig_y) < CFG_YLEN(chiplet_index) and
      unsigned(orig_chip_x) >= 0 and unsigned(orig_chip_x) < CFG_CHIPLET_COLS and
      unsigned(orig_chip_y) >= 0 and unsigned(orig_chip_y) < CFG_CHIPLET_ROWS
    then

      -- tile_id formula needs change KL
      tile_id := to_integer(unsigned(orig_x)) +
                 to_integer(unsigned(orig_y)) * CFG_XLEN(chiplet_index) +
                 CFG_CHIPLET_TILE_BASE(chiplet_index);
    ...
    case reg.state is
      ...
      when rcv_tail =>
        ...
        if noc5_data_void_out = '0' then
          ...
          packet_in_val  <= noc5_output_port(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 32 - 1 downto 0);
        end if;
...
-- FSM: Messages to NoC
-------------------------------------------------------------------------------
  snd_fsm : process (snd_reg, local_x, local_y, local_chip_x, local_chip_y,
    ...
    variable tile_id        : integer range 0 to CFG_SIP_TILES_NUM - 1;
  begin  -- process snd_fsm
    ...
    dest_chip_y := chip_y(tile_id);
    dest_chip_x := chip_x(tile_id);
    ...
    case reg.state is
        ...
        if noc5_stop_out = '0' then
          ...
          if packet_out = '1' then
            ...
            noc5_input_port <= create_header_misc(MISC_NOC_FLIT_SIZE,
                                             ...
                                             DVFS_MSG, local_chip_y, local_chip_x, dest_chip_y, dest_chip_x, (others => '0'));
          end if;
        end if;
      ...
      when snd_tail =>
        ...
        if noc5_stop_out = '0' then
          ...
          noc5_input_port(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 1 downto 0)                  <= std_logic_vector'(31 downto 0 => '0') & reg.val;
        end if;
