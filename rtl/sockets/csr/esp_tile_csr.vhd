...
--  constant DEFAULT_CPU_LOC_OVR : std_logic_vector(NCPU_TILE_MAX * 2 * (YX_WIDTH + CHIP_YX_WIDTH) downto 0) := (others => '0');
  constant DEFAULT_CPU_LOC_OVR : std_logic_vector(ESP_CSR_CPU_LOC_OVR_MSB - ESP_CSR_CPU_LOC_OVR_LSB downto 0) := (others => '0');
  -- CPU_Y(N-1) CPU_X(N-1) .... CPU_Y(0) CPU_X(0)    OVERWRITE DEFAULT FROM SOCMAP
  -- CPU_CHIP_Y(N-1) CPU_CHIP_X(N-1) ... CPU_CHIP_Y(0) CPU_CHIP_X(0)...
  ...
  begin
    ...
    if apbi.paddr(8 downto 7) = "11" then
      ...
      case csr_addr is
        ...
        when ESP_CSR_CPU_LOC_OVR_4_ADDR =>
          readdata <= (others => '0');
          readdata(27 downto 0) <=
            config_r(ESP_CSR_CPU_LOC_OVR_LSB + 156 downto ESP_CSR_CPU_LOC_OVR_LSB + 129);
--        when ESP_CSR_CPU_LOC_OVR_5_ADDR =>
--          readdata <=
--            config_r(ESP_CSR_CPU_LOC_OVR_LSB + 192 downto ESP_CSR_CPU_LOC_OVR_LSB + 161);
--        when ESP_CSR_CPU_LOC_OVR_6_ADDR =>
--          readdata <=
--            config_r(ESP_CSR_CPU_LOC_OVR_LSB + 224 downto ESP_CSR_CPU_LOC_OVR_LSB + 193);
        when others =>
          readdata <= (others => '0');
      end case;
    else
    ...
    if rstn = '0' then
      ...
      if apbi.paddr(8 downto 7) = "11" and (apbi.psel(pindex) and apbi.penable and apbi.pwrite) = '1' then
        case csr_addr is
          ...
          when ESP_CSR_CPU_LOC_OVR_4_ADDR =>
            config_r(ESP_CSR_CPU_LOC_OVR_LSB + 156 downto ESP_CSR_CPU_LOC_OVR_LSB + 129) <= apbi.pwdata(27 downto 0);
--          when ESP_CSR_CPU_LOC_OVR_5_ADDR =>
--            config_r(ESP_CSR_CPU_LOC_OVR_LSB + 192 downto ESP_CSR_CPU_LOC_OVR_LSB + 161) <= apbi.pwdata;
--          when ESP_CSR_CPU_LOC_OVR_6_ADDR =>
--            config_r(ESP_CSR_CPU_LOC_OVR_LSB + 224 downto ESP_CSR_CPU_LOC_OVR_LSB + 193) <= apbi.pwdata;
          when others => null;
        end case;
      end if;
    end if;
