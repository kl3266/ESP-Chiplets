      ...
      when receive_length =>
        if ahb_rcv_wrreq = '1' then
          if (hburst_reg = "001" and ahbsi.htrans = HTRANS_SEQ) then
            reg.count := to_integer(unsigned(ahbsi.hwdata(31 downto 0)));
          else
    ...
    case r.state is
      ...
      when get_length =>
        ...
        if(ahb_rcv_full or ahb_rcv_almost_full ) = '0' then
          ...
          if (hburst_reg = "001" and ahbsi.htrans = HTRANS_SEQ) then
            ahb_rcv_data_in <= ahbsi.hwdata(31 downto 0);
            v.cnt := to_integer(unsigned(ahbsi.hwdata(31 downto 0)));
            v.burst_mode := '1';
          else 
            ahb_rcv_data_in <= (others => '0');
            ahb_rcv_data_in(0) <= '1';
