...
entity intreq2noc is
  generic (
    ...
    cpu_chip_y : chip_yx_vec(0 to CFG_NCPU_TILE - 1);
    cpu_chip_x : chip_yx_vec(0 to CFG_NCPU_TILE - 1));
  port (
    rst : in std_ulogic;
    clk : in std_ulogic;
    ...
    cpu_loc_chip_y   : in chip_yx_vec(0 to CFG_NCPU_TILE - 1);
    cpu_loc_chip_x   : in chip_yx_vec(0 to CFG_NCPU_TILE - 1);
...
--pragma translate_on
  ...
  ncpu_irq_proxy_gen : for cpuid in ncpu - 1 downto 0 generate
    ...
    make_packet : process (irqi(cpuid), local_y, local_x, override_cpu_loc, cpu_loc_y, cpu_loc_x,
                          local_chip_y, local_chip_x, cpu_loc_chip_y, cpu_loc_chip_x)
      ...
      variable target_chip_y  : chip_yx;
      variable target_chip_x  : chip_yx;
    begin  -- process make_packet
      ...
      if override_cpu_loc = '1' and ncpu > 1 then
        ...
        target_chip_y := cpu_loc_chip_y(cpuid);
        target_chip_x := cpu_loc_chip_x(cpuid);
      else
        target_y := cpu_y(cpuid);
        target_x := cpu_x(cpuid);
        target_chip_y := cpu_chip_y(cpuid);
        target_chip_x := cpu_chip_x(cpuid);
      end if;

      header_v := (others                                                                       => '0');
      header_v := create_header_misc(MISC_NOC_FLIT_SIZE, local_y, local_x, target_y, target_x, msg_type, local_chip_y, local_chip_x, target_chip_y, target_chip_x, reserved);
      if GLOB_CPU_ARCH = ariane or GLOB_CPU_ARCH = ibex then
  ...
  arbitration_gen : if ncpu > 1 generate
    constant ARB_LOG2N  : integer := ncpu_log(ncpu); -- 2 when CPU = 3
    constant N_REQ_P2   : integer := 2**ARB_LOG2N;
    signal forwarding_req_p2 : std_logic_vector(N_REQ_P2 - 1 downto 0);
  begin
    ...
    forwarding_req_p2(ncpu-1 downto 0) <= forwarding_req; -- zero pad
    pad_upper_gen : if N_REQ_P2 > ncpu generate
      forwarding_req_p2(N_REQ_P2-1 downto ncpu) <= (others => '0');
    end generate pad_upper_gen;
