...
architecture rtl of noc2apb is
  ...
  signal request_chip_y, request_chip_x : chip_yx;
...
begin  -- rtl
  ...
  begin  -- process
    if rst = '0' then                   -- asynchronous reset (active low)
      request_y <= (others => '0');
      request_x <= (others => '0');
      request_chip_y <= (others => '0');
      request_chip_x <= (others => '0');
      ...
      if sample_request = '1' then
        ...
        request_chip_y <= get_chip_origin_y(MISC_NOC_FLIT_SIZE, misc_noc_flit_pad & apb_rcv_data_out);
        request_chip_x <= get_chip_origin_x(MISC_NOC_FLIT_SIZE, misc_noc_flit_pad & apb_rcv_data_out);
  ...
  header <= create_header_misc(MISC_NOC_FLIT_SIZE, local_y, local_x, request_y, request_x, RSP_REG_RD, local_chip_y, local_chip_x, request_chip_y, request_chip_x, (others => '0'))(MISC_NOC_FLIT_SIZE - 1 downto 0);
  ...
  apb_roundtrip: process (apb_state, apbo, apb_rcv_empty, apb_rcv_data_out,
                          apb_snd_full, request_y, request_x, request_chip_y,
                          request_chip_x, request_msg_type,
  ...
  begin  -- process apb_roundtrip
    ...
    prdata <= (others => '0');
    reply_v := prdata_reg;
    tail_v := PREAMBLE_TAIL & std_logic_vector'(MISC_NOC_FLIT_SIZE - PREAMBLE_WIDTH - 32 - 1 downto 0 => '0') & reply_v;
    tail <= tail_v;
