...
entity llc_wrapper is
  generic (
    ...
    tile_cache_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1);
    tile_dma_id   : attribute_vector(0 to CFG_SIP_TILES_NUM - 1);
    eth_dma_id    : integer;
    dma_y         : yx_vec(0 to 2**NLLC_MAX_LOG2 - 1);
    dma_x         : yx_vec(0 to 2**NLLC_MAX_LOG2 - 1);
    dma_chip_y    : chip_yx_vec(0 to 2**NLLC_MAX_LOG2 - 1);
    dma_chip_x    : chip_yx_vec(0 to 2**NLLC_MAX_LOG2 - 1);
...
-----------------------------------------------------------------------------
  ...
  constant RSP_OUT_REG_DEFAULT : rsp_out_reg_type := (
    ...
    dest_x     => (others => '0'),
    dest_y     => (others => '0'),
    dest_chip_x     => (others => '0'),
    dest_chip_y     => (others => '0'),
    reserved   => (others => '0'),
    stall      => '0',
    asserts    => (others => '0'));
  ...
  type req_in_reg_type is record
    ...
    origin_x : local_yx;
    origin_y : local_yx;
    origin_chip_x : chip_yx;
    origin_chip_y : chip_yx;
    tile_id  : integer;
  ...
  constant REQ_IN_REG_DEFAULT : req_in_reg_type := (
    ...
    origin_x => (others => '0'),
    origin_y => (others => '0'),
    origin_chip_x => (others => '0'),
    origin_chip_y => (others => '0'),
    tile_id  => 0,
    asserts  => (others => '0'));
...
-----------------------------------------------------------------------------
  fsm_req_in : process (req_in_reg, llc_req_in_ready,
    ...
    variable chip_idx : integer range 0 to CFG_CHIPLET_NUM-1;
    ...
    case reg.state is
      ...
      when rcv_header =>
        ...
        if coherence_req_empty = '0' and preamble(1) = '1' then
          ...
          -- chiplet
          reg.origin_chip_x := get_chip_origin_x(COH_NOC_FLIT_SIZE, coh_noc_flit_pad & coherence_req_data_out);
          reg.origin_chip_y := get_chip_origin_y(COH_NOC_FLIT_SIZE, coh_noc_flit_pad & coherence_req_data_out);

          chip_idx := to_integer(unsigned(reg.origin_chip_y)) * CFG_CHIPLET_COLS + to_integer(unsigned(reg.origin_chip_x));

          if unsigned(reg.origin_x) >= 0 and unsigned(reg.origin_x) < CFG_XLEN(chip_idx) and
             unsigned(reg.origin_y) >= 0 and unsigned(reg.origin_y) < CFG_YLEN(chip_idx) and
             unsigned(reg.origin_chip_x) >= 0 and unsigned(reg.origin_chip_x) < CFG_CHIPLET_COLS and
             unsigned(reg.origin_chip_y) >= 0 and unsigned(reg.origin_chip_y) < CFG_CHIPLET_ROWS
          then

            reg.tile_id := to_integer(unsigned(reg.origin_x)) +
                           to_integer(unsigned(reg.origin_y)) * CFG_XLEN(chip_idx) +
                           CFG_CHIPLET_TILE_BASE(chip_idx);
    ...
    case reg.state is
      ...
      when rcv_header =>
        ...
        if dma_rcv_empty = '0' and dma_preamble(1) = '1' then
          ...
          reg.origin_chip_x := get_chip_origin_x(DMA_NOC_FLIT_SIZE, dma_noc_flit_pad & dma_rcv_data_out);
          reg.origin_chip_y := get_chip_origin_y(DMA_NOC_FLIT_SIZE, dma_noc_flit_pad & dma_rcv_data_out);
    ...
    case reg.state is
      ...
      when rcv_header =>
        ...
        if coherence_rsp_rcv_empty = '0' and preamble(1) = '1' then
          ...
          reg.origin_chip_x := get_chip_origin_x(COH_NOC_FLIT_SIZE, coh_noc_flit_pad & coherence_rsp_rcv_data_out);
          reg.origin_chip_y := get_chip_origin_y(COH_NOC_FLIT_SIZE, coh_noc_flit_pad & coherence_rsp_rcv_data_out);
          ...
          if unsigned(reg.origin_x) >= 0 and unsigned(reg.origin_x) < CFG_XLEN(chip_idx) and
             ...
             unsigned(reg.origin_chip_y) >= 0 and unsigned(reg.origin_chip_y) < CFG_CHIPLET_ROWS
          then
            reg.tile_id := to_integer(unsigned(reg.origin_x)) + to_integer(unsigned(reg.origin_y)) * CFG_XLEN(chip_idx) + 
                          CFG_CHIPLET_TILE_BASE(chip_idx);
            if tile_cache_id(reg.tile_id) >= 0 then
              reg.req_id := std_logic_vector(to_unsigned(tile_cache_id(reg.tile_id), NL2_MAX_LOG2));
            end if;
...
-------------------------------------------------------------------------------
  fsm_fwd_out : process (fwd_out_reg, coherence_fwd_full,
                         ...
                         llc_fwd_out_data_line, local_y, local_x, local_chip_y, local_chip_x) is

    variable reg       : fwd_out_reg_type;
    variable dest_init : integer;
    variable dest_x    : local_yx;
    variable dest_y    : local_yx;
    variable dest_chip_x, dest_chip_y : chip_yx;
          ...
          if llc_fwd_out_valid = '1' then
            ...
            if llc_fwd_out_data_dest_id >= "0" then
              ...
              if dest_init >= 0 then
                ...
                dest_chip_x := cache_chip_x(dest_init);
                dest_chip_y := cache_chip_y(dest_init);
              end if;
            end if;
            ...
            coherence_fwd_data_in <= create_header(COH_NOC_FLIT_SIZE, local_y, local_x, dest_y, dest_x,
                                                   reg.coh_msg, local_chip_y, local_chip_x, dest_chip_y, dest_chip_x, req_id);
...
-------------------------------------------------------------------------------
  fsm_rsp_out : process (rsp_out_reg, coherence_rsp_snd_full,
                         ...
                         llc_rsp_out_data_req_id, llc_rsp_out_data_dest_id, llc_rsp_out_data_word_mask, local_y, local_x, local_chip_y, local_chip_x) is
          ...
          if coherence_rsp_snd_full = '0' then
            ...
            coherence_rsp_snd_data_in <=
              create_header(COH_NOC_FLIT_SIZE, local_y, local_x, dest_y, dest_x,
                            mix_msg, local_chip_y, local_chip_x, dest_chip_y, dest_chip_x, reserved);
            ...
            reg.dest_chip_x := dest_chip_x;
            reg.dest_chip_y := dest_chip_y;
        ...
        if coherence_rsp_snd_full = '0' then
          ...
          coherence_rsp_snd_data_in <=
            create_header(COH_NOC_FLIT_SIZE, local_y, local_x, reg.dest_y, reg.dest_x,
                          mix_msg, local_chip_y, local_chip_x, reg.dest_chip_y, reg.dest_chip_x, reg.reserved);
...
-------------------------------------------------------------------------------
  dma_fsm_rsp_out : process (dma_rsp_out_reg, dma_snd_full,
                             ...
                             local_y, local_x, local_chip_y, local_chip_x) is
    ...
    case reg.state is
        ...
        if llc_dma_rsp_out_valid = '1' then
          ...
          if llc_dma_rsp_out_data_req_id >= "0" then
            ...
            if dest_init >= 0 then
              ...
              dest_chip_x := dma_chip_x(dest_init);
              dest_chip_y := dma_chip_y(dest_init);
            end if;
          end if;
          ...
          if dma_snd_full = '0' then
            ...
            dma_snd_data_in <= create_header(DMA_NOC_FLIT_SIZE, local_y, local_x, dest_y,
                                             dest_x, mix_msg, local_chip_y, local_chip_x, dest_chip_y, dest_chip_x, (others => '0'));
      ...
      when send_header_dma_stall =>
        ...
        if dma_snd_full = '0' then
          ...
          dma_snd_wrreq   <= '1';
          dma_snd_data_in <= create_header(DMA_NOC_FLIT_SIZE, local_y, local_x, reg.dest_y, reg.dest_x, mix_msg, local_chip_y, local_chip_x, reg.dest_chip_y, reg.dest_chip_x, (others => '0'));
