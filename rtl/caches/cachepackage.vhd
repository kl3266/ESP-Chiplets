...
package cachepackage is
  ...
  function make_header (coh_msg     : coh_msg_t; mem_info : tile_mem_info_vector(0 to MEM_ID_RANGE_MSB);
                        ...
                        local_chip_x : chip_yx; local_chip_y : chip_yx;
...
package body cachepackage is
  ...
  begin
    ...
    if to_req = '0' then
      ...
      dest_x := mem_info(0).x;
      dest_y := mem_info(0).y;
      dest_chip_x := mem_info(0).chip_x;
      dest_chip_y := mem_info(0).chip_y;
      ...
      if mem_num /= 1 then
        for i in 0 to mem_num - 1 loop
          if ((addr(LINE_ADDR_BITS - 1 downto LINE_ADDR_BITS - 12)
            ...
            dest_x := mem_info(i).x;
            dest_y := mem_info(i).y;
            dest_chip_x := mem_info(i).chip_x;
            dest_chip_y := mem_info(i).chip_y;
          end if;
        end loop;
      end if;
      ...
      if req_id >= "0" then
        ...
        if dest_init >= 0 then
          ...
          dest_chip_x := cache_chip_x(dest_init); --KL
    ...
    -- Chiplet Header
    header := create_header(COH_NOC_FLIT_SIZE, local_y, local_x, dest_y, dest_x, noc_msg, local_chip_y, local_chip_x, dest_chip_y, dest_chip_x, reserved);
  ...
  begin
    ...
    header := create_header(COH_NOC_FLIT_SIZE, local_y, local_x, dest_y, dest_x, '0' & coh_msg, local_chip_y, local_chip_x, dest_chip_y, dest_chip_x, reserved);
