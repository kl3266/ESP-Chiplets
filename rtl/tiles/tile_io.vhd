...
entity tile_io is
  generic (
    ...
    this_has_dco : integer range 0 to 2 := 0; --0: no DCO 1: tile and NoC DCO, 2: NoC DCO only
...
architecture rtl of tile_io is
  ...
  signal cpu_loc_chip_y   : chip_yx_vec(0 to CFG_NCPU_TILE - 1);
  signal cpu_loc_chip_x   : chip_yx_vec(0 to CFG_NCPU_TILE - 1);
...
--    others => '0');
  ...
  begin
    if chiplet_index = 0 then
      ahb := (
        0 => '1', -- bootrom/bootram visible on this tile
        1 => '1', -- ahb2apb bridge
        2 => to_std_logic(GLOB_CPU_RISCV * GLOB_CPU_AXI),  -- clint (if present)
        3 => to_std_logic(CFG_SVGA_ENABLE),                -- frame buffer (if present)
        others => '0');
    else
      ahb := (
        0 => '1', -- bootrom/bootram visible on this tile
    ...
    return ahb;
  end function set_ahb_constants;

  function set_apb_constants(constant chiplet_index : integer)
    return std_logic_vector is
  ...
  begin
    ...
    return apb;
  end function set_apb_constants;

  constant this_local_ahb_en : std_logic_vector(0 to NAHBSLV - 1) := set_ahb_constants(chiplet_index);
  constant this_local_apb_en : std_logic_vector(0 to NAPBSLV - 1) := set_apb_constants(chiplet_index);

  function mask_out_local (
    constant m   : std_logic_vector(0 to NAPBSLV - 1);
    constant idx : integer) return std_logic_vector is
    variable r : std_logic_vector(0 to NAPBSLV - 1);
  begin
    r := m;
...
-- pragma translate_on
  ...
  riscv_plic_gen: if GLOB_CPU_ARCH = ariane or GLOB_CPU_ARCH = ibex generate
    ...
    fsm_intr_ack: process (intr_ack_state, noc_apbi_wirq, plic_pready, interrupt_ack_full, header) is
      ...
      variable dest_chip_y, dest_chip_x : chip_yx;
    begin  -- process fsm_intr_ack
      ...
      for i in 0 to CFG_SIP_TILES_NUM - 1 loop
        if tile_irq_type(i) = 1 and
            tile_apb_irq(i) = to_integer(unsigned(noc_apbi_wirq.pwdata)) - 1 then
          irq_pwdata_hit := '1';
          dest_y := tile_y(i);
          dest_x := tile_x(i);
          dest_chip_y := chip_y(i);
          dest_chip_x := chip_x(i);
        end if;
      end loop;  -- i
        ...
        when idle =>
          ...
          if (plic_pready = '1' and noc_apbi_wirq.penable = '1' and noc_apbi_wirq.psel(2) = '1' and
            ...
            header_reg := create_header_misc(MISC_NOC_FLIT_SIZE, this_local_y, this_local_x, dest_y, dest_x,
                                        INTERRUPT, this_local_chip_y, this_local_chip_x, dest_chip_y, dest_chip_x, (others => '0'));
  ...
  apb2noc_1 : apb2noc
    generic map (
      ...
      apb_slv_y   => apb_slv_y,
      apb_slv_x   => apb_slv_x,
      apb_slv_chip_y => apb_slv_chip_y,
      apb_slv_chip_x => apb_slv_chip_x)
    port map (
      rst                     => rst,
      clk                     => tile_clk,
      local_chip_y            => this_local_chip_y,
  ...
  cpu_loc_ovr_gen: for i in 0 to CFG_NCPU_TILE - 1 generate -- KL NEED TO MODIFY!!!!!! cpu_loc_chip_x, cpu_loc_chip_y
    ...
    cpu_loc_chip_x(i) <= tile_config_int(ESP_CSR_CPU_LOC_OVR_LSB + 1 + 12 * 2 * YX_WIDTH + i * CHIP_YX_WIDTH * 2 + CHIP_YX_WIDTH - 1 downto ESP_CSR_CPU_LOC_OVR_LSB + 1 + 12 * 2 * YX_WIDTH + i * CHIP_YX_WIDTH * 2); -- 12 is 3 addr worth of MAX CPU TILES from tile coordinates, but max is constrained by 5 * 2 addr worth of 10 MAX CPU TILES
    cpu_loc_chip_y(i) <= tile_config_int(ESP_CSR_CPU_LOC_OVR_LSB + 1 + 12 * 2 * YX_WIDTH + i * CHIP_YX_WIDTH * 2 + CHIP_YX_WIDTH * 2 - 1 downto ESP_CSR_CPU_LOC_OVR_LSB + 1 + 12 * 2 * YX_WIDTH + i * CHIP_YX_WIDTH * 2 + CHIP_YX_WIDTH);
  ...
  intreq2noc_1 : intreq2noc
    generic map (
      ...
      cpu_y => cpu_y,
      cpu_x => cpu_x,
      cpu_chip_y => cpu_chip_y,
      cpu_chip_x => cpu_chip_x)
    port map (
      rst                => rst,
      clk                => tile_clk,
      local_chip_y       => this_local_chip_y,
      ...
      cpu_loc_chip_y     => cpu_loc_chip_y,
      cpu_loc_chip_x     => cpu_loc_chip_x,
      cpu_loc_y          => cpu_loc_y,
      cpu_loc_x          => cpu_loc_x,
      irqi               => irqi,
      irqo               => irqo,
...
--  end generate large_bus;
  ...
  -- KL TODO later on: not use narrow_to_large/large_to_narrow and make things work with noc2ahbmst narrow_noc = 0. When narrow_noc = 1, same 32-bit data repeated twice in the same flit. With narrow_noc = 0, two 32-bit data is in 1 flit and other modules cannot handle this.
