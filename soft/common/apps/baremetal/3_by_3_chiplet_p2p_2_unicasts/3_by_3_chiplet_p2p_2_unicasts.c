...
//        esp_p2p_set_mcast_ndests(dev, mcast_ndests);      // Number of downstream consumers
//        esp_p2p_set_mcast_packet(dev, mcast_packet);      // Enable/disable packet mode
//        esp_p2p_set_mcast_packet_size(dev, mcast_packet_size); // Size for packet mode
    }
    ...
    if (p2p_load) { // Configure as source/load point
        ...
        if (p2p_src) { // Check if source pointer is valid
          ...
          esp_p2p_set_chip_y(dev, 0, esp_get_chip_y(p2p_src)); // Set source Y coordinate
          esp_p2p_set_chip_x(dev, 0, esp_get_chip_x(p2p_src)); // Set source X coordinate

        } else {
//        printf("Warning: p2p_load enabled but p2p_src is NULL\n");
          // Set default source coordinates or handle error if necessary
        ...
        // Configure packet settings for load side if needed (often 0)
...
int main()
{
    ...
    const int ndev = 6;      // Total number of devices available in the system

//    printf("\n--- Starting ESP Scatter-Gather Multicast Test Suite ---\n");
//    printf("System Configuration: ndev = %d\n", ndev);

    // Define test parameters
    int batches[] = {16};          // BATCH values to test
//    int batches[] = {64, 256};
...
//    struct esp_device *devs = NULL;
    int devices_0[] = {0, 8, 1, 3, 4};
    int devices_1[] = {6, 2, 5, 7};
    //int devices_2[] = {2, 4};
...
//        printf("    PT_ADDRESS=0x%lx, PT_NCHUNK=%d, PT_SHIFT=%d\n", (uintptr_t)ptables[devices_0[i]], chunks_per_dev, CHUNK_SHIFT);
    }
