...
#define CACHELINE_SIZE 256 // Alignment must be a power of two
...
int main()
{
    ...
    const int ndev = 17;      // Total number of devices available in the system

    // Define test parameters
    int batches[] = {255};          // BATCH values to test
    int multicasts[] = {4};     // NUM_MULTICAST values (Max = ndev-1)
    int mcast_packets[] = {0};
    ...
    for (int u_idx = 0; u_idx < num_pkt_vals; u_idx++) { // Loop MCAST_PACKET
        ...
        for (int i_idx = 0; i_idx < num_batch_vals; i_idx++) { // Loop BATCH
            ...
            for (int j_idx = 0; j_idx < num_mc_vals; j_idx++) { // Loop NUM_MULTICAST
                ...
                count[u_idx][k][l] = test(current_batch, fixed_tokens, current_mcast_packet, current_num_multicast, ACC_COH_NONE);
...
//    }
    ...
    int banks[] = {1,1,1,2,2,2,3,3,3,4,4,4,4,4,3,3,3};
    for (int i = 0; i < ndev; i++) {
