...
#define esp_get_chip_y(_dev) (CHIP_YX_MASK_CHIP_YX & (ioread32be(_dev->iomem + CHIP_YX_REG) >> CHIP_YX_SHIFT_CHIP_Y))
#define esp_get_chip_x(_dev) (CHIP_YX_MASK_CHIP_YX & (ioread32be(_dev->iomem + CHIP_YX_REG) >> CHIP_YX_SHIFT_CHIP_X))
#define esp_p2p_reset(_dev) iowrite32be(0, _dev->iomem + P2P_REG)
#define esp_p2p_chip_reset(_dev) iowrite32be(0, _dev->iomem + P2P_CHIP_REG)
	...
	list_for_each(ele, &esp_devices) {
		...
		if (!strncmp(src_name, dev->dev->kobj.name, strlen(dev->dev->kobj.name))) {
            ...
            if (is_yx) {
                ...
                esp_chip_yx_reg_set_y(esp, chip_y, 5 * (src_index / 5), src_index % 5);
                esp_chip_yx_reg_set_x(esp, chip_x, 5 * (src_index / 5), src_index % 5);
            } else {
