...
void mmap_monitors()
{
	...
	monitor_base_ptr = mmap(NULL, SOC_NTILES * MONITOR_TILE_SIZE
...
{
	munmap(monitor_base_ptr, SOC_NTILES * MONITOR_TILE_SIZE);
}
#endif
...
#include "soc_locs.h"
	...
	if (args.read_mode == ESP_MON_READ_SINGLE){
      ...
      chip_idx = mem_locs[t].chip_row * CHIPLET_COLS + mem_locs[t].chip_col;
			vals->ddr_accesses[t] =
        read_monitor(mem_locs[t].row * chip_tiles[chip_idx].tile_col + mem_locs[t].col + tile_base[chip_idx], MON_DDR_WORD_TRANSFER_INDEX);
        //read_monitor(mem_locs[t].row * SOC_COLS + mem_locs[t].col, MON_DDR_WORD_TRANSFER_INDEX);
		//mem_reqs
    }
		for (t = 0; t < SOC_NMEM; t++){
      chip_idx = mem_locs[t].chip_row * CHIPLET_COLS + mem_locs[t].chip_col;
      tile = mem_locs[t].row * chip_tiles[chip_idx].tile_col + mem_locs[t].col + tile_base[chip_idx];
      ...
      chip_idx = cpu_locs[t].chip_row * CHIPLET_COLS + cpu_locs[t].chip_col;
      tile = cpu_locs[t].row * chip_tiles[chip_idx].tile_col + cpu_locs[t].col + tile_base[chip_idx];
      //tile = cpu_locs[t].row * SOC_COLS + cpu_locs[t].col;
		...
		for (t = 0; t < SOC_NACC; t++) {
			if (acc_has_l2[t]) {
        chip_idx = acc_locs[t].chip_row * CHIPLET_COLS + acc_locs[t].chip_col;
        tile = acc_locs[t].row * chip_tiles[chip_idx].tile_col + acc_locs[t].col + tile_base[chip_idx];
...
//        tile = mem_locs[t].row * SOC_COLS + mem_locs[t].col;
      ...
      chip_idx = acc_locs[args.acc_index].chip_row * CHIPLET_COLS + acc_locs[args.acc_index].chip_col;
      tile = acc_locs[args.acc_index].row * chip_tiles[chip_idx].tile_col + acc_locs[args.acc_index].col + tile_base[chip_idx];
