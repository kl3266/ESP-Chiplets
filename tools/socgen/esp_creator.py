...
class AdvancedFrame(Frame):
  ...
  def __init__(self, soc, top_frame, main_frame):
    ...
    if soc.TECH_TYPE == "asic" or soc.TECH == "inferred" or soc.ESP_EMU_TECH != "none":
      ...
      Label(advanced_config_frame, text = "Dual external clocks (NoC and Tile)", fg="darkgreen").grid(row=1, column=1)
...
#      soc.clk_str.set(0)
    Label(advanced_config_frame, text = "Chiplet Rows: ").grid(row=4, column=1)
    OptionMenu(advanced_config_frame, soc.chiplet_rows, *chiplet_dim_choices, command=main_frame.update_noc_config).grid(row=4, column=2)
    Label(advanced_config_frame, text = "Chiplet Cols: ").grid(row=5, column=1)
    OptionMenu(advanced_config_frame, soc.chiplet_cols, *chiplet_dim_choices, command=main_frame.update_noc_config).grid(row=5, column=2)
