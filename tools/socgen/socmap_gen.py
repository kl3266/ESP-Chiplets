...
class soc_config:
  ...
  def __init__(self, soc):
    ...
    self.ncpu = soc.noc.get_cpu_num(soc) * soc.chiplet_rows.get() * soc.chiplet_cols.get()
    self.nmem = soc.noc.get_mem_num(soc) * soc.chiplet_rows.get() * soc.chiplet_cols.get()
    self.nslm = soc.noc.get_slm_num(soc) * soc.chiplet_rows.get() * soc.chiplet_cols.get()
    ...
    self.nacc = soc.noc.get_acc_num(soc) * soc.chiplet_rows.get() * soc.chiplet_cols.get()
    self.ntiles = soc.noc.rows * soc.noc.cols * soc.chiplet_rows.get() * soc.chiplet_cols.get()
    self.ntiles_chiplet = soc.noc.rows * soc.noc.cols
    ...
    self.tiles_chiplet = [tile_info() for x in range(0, self.ntiles_chiplet)]
    ...
    for cx in range(soc.chiplet_rows.get()):
      for cy in range (soc.chiplet_cols.get()):
        for x in range(soc.noc.rows):
          for y in range(soc.noc.cols):
            ...
            t = cx * soc.chiplet_cols.get() * soc.noc.cols * soc.noc.rows + x * soc.chiplet_cols.get() * soc.noc.cols + cy * soc.noc.cols + y

            self.tiles[t].row = cx * soc.noc.rows + x
            self.tiles[t].col = cy * soc.noc.cols + y
            ...
            if cx == 0 and cy == 0:
              self.tiles_chiplet[y + x * soc.noc.cols] = self.tiles[t]

    self.misc_id = 0
    for x in range(soc.noc.rows):
      for y in range(soc.noc.cols):
        if self.tiles[x * soc.chiplet_cols.get() * soc.noc.cols + y].type == "misc":
          self.misc_id = x * soc.chiplet_cols.get() * soc.noc.cols + y
          break
      else:
        continue
      break
...
def print_constants(fp, soc, esp_config):
  ...
  fp.write("  ------ Chiplets\n")
  fp.write("  constant CFG_CHIPLET_ROWS : integer := " + str(soc.chiplet_rows.get()) + ";\n")
  fp.write("  constant CFG_CHIPLET_COLS : integer := " + str(soc.chiplet_cols.get()) + ";\n")
  fp.write("  constant CFG_SIP_TILES_NUM : integer := CFG_XLEN * CFG_YLEN * CFG_CHIPLET_ROWS * CFG_CHIPLET_COLS;\n\n")
  ...
  fp.write("  constant CFG_NMEM_TILE_CHIPLET : integer := " + str(soc.noc.get_mem_num(soc)) + ";\n")
...
def print_mapping(fp, soc, esp_config):
  ...
  fp.write("  type tile_hlscfg_array is array (0 to CFG_SIP_TILES_NUM - 1) of hlscfg_t;\n")

  #
  fp.write("  -- Array of device IDs\n")
  fp.write("  type tile_device_array is array (0 to CFG_SIP_TILES_NUM - 1) of devid_t;\n")

  #
  fp.write("  -- Array of I/O-bus indices\n")
  fp.write("  type tile_idx_array is array (0 to CFG_SIP_TILES_NUM - 1) of integer range 0 to NAPBSLV - 1;\n")
  ...
  fp.write("  type tile_irq_array is array (0 to CFG_SIP_TILES_NUM - 1) of integer range 0 to NAHBIRQ - 1;\n")

  #
  fp.write("  -- Array of 12-bit addresses\n")
  fp.write("  type tile_addr_array is array (0 to CFG_SIP_TILES_NUM - 1) of integer range 0 to 4095;\n")

  #
  fp.write("  -- Array of flags\n")
  fp.write("  type tile_flag_array is array (0 to CFG_SIP_TILES_NUM - 1) of integer range 0 to 1;\n")
  ...
  fp.write("  constant cpu_dvfs_paddr : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  ...
  fp.write("  constant tile_cpu_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
  ...
  fp.write("  constant tile_cache_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
  ...
  fp.write("  constant l2_cache_pindex : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
  ...
  fp.write("  constant tile_has_l2 : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
  ...
  fp.write("  constant tile_has_tdvfs : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
  ...
  fp.write("  constant tile_llc_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
  ...
  fp.write("  constant llc_cache_pindex : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
  ...
  fp.write("  constant tile_slm_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in  range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.slm_id != -1:
  ...
  #
  fp.write("  -- Get shared-local memory tile ID from tile ID\n")
  fp.write("  constant tile_slmddr_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in  range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.slmddr_id != -1:
  ...
  fp.write("  constant tile_mem_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in  range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.mem_id != -1:
  ...
  fp.write("  constant tile_csr_pindex : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
  ...
  fp.write("  constant tile_acc_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.acc.id != -1:
  ...
  fp.write("  constant acc_tile_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.acc.id != -1:
  ...
  fp.write("  constant io_tile_id : integer := " + str(esp_config.misc_id) + ";\n\n")
  ...
  fp.write("  constant tile_dma_id : attribute_vector(0 to CFG_SIP_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
  ...
  for i in range(0, esp_config.ntiles_chiplet):
    type = 0
    if esp_config.tiles_chiplet[i].type == "cpu":
      type = 1
    if esp_config.tiles_chiplet[i].type == "acc":
      type = 2
    if esp_config.tiles_chiplet[i].type == "misc":
      type = 3
    if esp_config.tiles_chiplet[i].type == "mem":
      type = 4
    if esp_config.tiles_chiplet[i].type == "slm":
      type = 5
    if esp_config.tiles_chiplet[i].type == "slmddr":
      type = 6
  ...
  #
  fp.write("  constant esp_init_sequence : attribute_vector(0 to CFG_SIP_TILES_NUM + CFG_NCPU_TILE - 1) := (\n")
  fp.write("    ")
  # Set tile_id to adjacent tiles starting from tile_io
  for i in list(range(esp_config.tiles[esp_config.misc_id].row, -1, -1)) + list(range(esp_config.tiles[esp_config.misc_id].row + 1, soc.noc.rows * soc.chiplet_rows.get())):
    for j in list(range(esp_config.tiles[esp_config.misc_id].col, -1, -1)) + list(range(esp_config.tiles[esp_config.misc_id].col + 1, soc.noc.cols * soc.chiplet_cols.get())):
      tile_id = i * soc.noc.cols * soc.chiplet_cols.get() + j
      if tile_id != esp_config.misc_id:
        fp.write(", ")
