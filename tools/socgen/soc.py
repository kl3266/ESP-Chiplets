...
class SoC_Config():
  ...
  def read_config(self, temporary):
    ...
    line = fp.readline()
    item = line.split();
    self.chiplet_rows.set(int(item[2]))
    line = fp.readline()
    item = line.split()
    self.chiplet_cols.set(int(item[2]))
    line = fp.readline()
    return 0
  ...
  def write_config(self, dsu_ip, dsu_eth):
    ...
    fp.write("CONFIG_CHIPLET_ROWS = " + str(self.chiplet_rows.get()) + "\n")
    fp.write("CONFIG_CHIPLET_COLS = " + str(self.chiplet_cols.get()) + "\n")
  ...
  def __init__(self, ARCH_BITS, TECH_TYPE, TECH, LINUX_MAC, LEON3_STACK, FPGA_BOARD, EMU_TECH, EMU_FREQ, temporary):
    ...
    self.chiplet_rows = IntVar()
    self.chiplet_cols = IntVar()
