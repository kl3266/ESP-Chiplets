...
struct Rect { int r, c, rs, cs; };  // row, column, rowSpan, colSpan in global units
...
public:
  ...
  void build_geometry();
  inline const Rect& tile_rect(int tid) const {return geom_[tid];}
  inline int global_rows_units() const {return grid_rows_units_; }
  inline int global_cols_units() const {return grid_cols_units_; }
  inline int neighbor(int tid, int dir) const {return neighbors_[tid][dir]; }
...
private:
  std::vector<Rect> geom_;
  int neighbors_[TILES_NUM][4]; // N,S,W,E per tile, -1 if none
  int grid_rows_units_ = 0;
  int grid_cols_units_ = 0;
  static constexpr int MICRO_ = 6;  // micro cells per global unit
