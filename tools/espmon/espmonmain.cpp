...
void EspMonMain::show_soc()
{
    ...
    const Rect& g = tile_rect(i);
		ui->layout_soc->addWidget(btn, g.r, g.c, g.rs, g.cs);
	}
}
...
void EspMonMain::build_geometry()
{
  int chip_rows = 0, chip_cols = 0;
  for (int t = 0; t < TILES_NUM; t++) {
    chip_rows = std::max(chip_rows, int(tiles_t[t].chip.position.y) + 1);
    chip_cols = std::max(chip_cols, int(tiles_t[t].chip.position.x) + 1);
  }
  chip_rows = std::max(chip_rows, 1);
  chip_cols = std::max(chip_cols, 1);

  struct CH { int w = 0, h = 0; };
  auto CIDX = [chip_cols](int r, int c) { return r * chip_cols + c; };
  std::vector<CH> chip_sz(chip_rows * chip_cols);
  for (int t = 0; t < TILES_NUM; ++t) {
    const auto& ti = tiles[t];
    CH &cs = chip_sz[CIDX(ti.chip_position.y, ti.chip_position.x)];
    cs.w = std::max(cs.w, int(ti.position.x) + int(ti.span.w));
    cs.h = std::max(cs.h, int(ti.position.y) + int(ti.span.h));
  }

  std::vector<int> col_w(chip_cols, 0), row_h(chip_rows, 0);
  for (int r = 0; r < chip_rows; ++r) {
    for (int c = 0; c < chip_cols; ++c) {
      const CH& cs = chip_sz[CIDX(r,c)];
      row_h[r] = std::max(row_h[r], cs.h);
      col_w[c] = std::max(col_w[c], cs.w);
    }
  }
  std::vector<int> oy(chip_rows, 0), ox(chip_cols, 0);
  for (int r = 1; r < chip_rows; ++r) oy[r] = oy[r-1] + row_h[r-1];
  for (int c = 1; c < chip_cols; ++c) ox[c] = ox[c-1] + col_w[c-1];

  grid_rows_units_ = oy.back() + row_h.back(); // total height
  grid_cols_units_ = ox.back() + col_w.back(); // total width

  geom_.assign(TILES_NUM, Rect{0,0,1,1});
  for (int t = 0; t < TILES_NUM; ++t) {
    const auto& ti = tiles[t];
    Rect R;
    R.r  = oy[ti.chip_position.y] + int(ti.position.y);
    R.c  = ox[ti.chip_position.x] + int(ti.position.x);
    R.rs = std::max(1, int(ti.span.h));
    R.cs = std::max(1, int(ti.span.w));
    geom_[t] = R;
  }

  for (int t = 0; t < TILES_NUM; ++t)
    for (int d = 0; d < 4; ++d) neighbors_[t][d] = -1;

  auto overlap = [](int a0,int a1,int b0,int b1){ return !(a1 <= b0 || b1 <= a0); };

  for (int i = 0; i < TILES_NUM; ++i) {
    const Rect A = geom_[i];
    for (int j = 0; j < TILES_NUM; ++j) {
      if (i == j) continue;
      const Rect B = geom_[j];
      // North: B bottom touches A top
      if (B.r + B.rs == A.r && overlap(A.c, A.c + A.cs, B.c, B.c + B.cs)) neighbors_[i][0] = j;
      // South: A bottom touches B top
      if (A.r + A.rs == B.r && overlap(A.c, A.c + A.cs, B.c, B.c + B.cs)) neighbors_[i][1] = j;
      // West:  B right  touches A left
      if (B.c + B.cs == A.c && overlap(A.r, A.r + A.rs, B.r, B.r + B.rs)) neighbors_[i][2] = j;
      // East:  A right  touches B left
      if (A.c + A.cs == B.c && overlap(A.r, A.r + A.rs, B.r, B.r + B.rs)) neighbors_[i][3] = j;
    }
  }
}
...
void EspMonMain::show_heatmap()
{
  ...
  const int N = MICRO_;
	const int H = global_rows_units();
	const int W = global_cols_units();
	...
	for (int y = 0; y < H * N; y++)
		for (int x = 0; x < W * N; x++)
			for (int h = 0; h < NOCS_NUM; h++) {
				...
				sizePolicy1.setHeightForWidth(w1->sizePolicy().hasHeightForWidth());
...
void EspMonMain::update_heatmap(int noc, float max, uint32_t (&probes_queue)[NOCS_NUM][TILES_NUM][DIRECTIONS])
{
	...
	for (int i = 0; i < TILES_NUM; i++) {
		...
		// Anchor inside the tile cell (same offsets you used before)
		int y0 = N * g.r + 2;
		int x0 = N * g.c + 2;
		...
		if (neighbor(i, 0) != -1) {
			...
			router_is_active[noc][i] += probes_queue[noc][i][0];
			for (int y = 1; y < N - 1; y++) {
				QPalette Pal(palette()); Pal.setColor(QPalette::Background, col);
				wid_map[noc][(y0 - y) * N * W + x0]->setPalette(Pal);
				wid_map1[noc][(y0 - y) * N * W + x0]->setPalette(Pal);
			}
		}
		...
		if (neighbor(i, 1) != -1) {
			...
			router_is_active[noc][i] += probes_queue[noc][i][1];
			for (int y = 1; y < N - 1; y++) {
				QPalette Pal(palette()); Pal.setColor(QPalette::Background, col);
				wid_map[noc][(y0 + y) * N * W + x0]->setPalette(Pal);
				wid_map1[noc][(y0 + y) * N * W + x0]->setPalette(Pal);
		...
		if (neighbor(i, 2) != -1) {
			...
			router_is_active[noc][i] += probes_queue[noc][i][2];
			for (int x = 1; x < N - 1; x++) {
				QPalette Pal(palette()); Pal.setColor(QPalette::Background, col);
				wid_map[noc][y0 * N * W + x0 - x]->setPalette(Pal);
				wid_map1[noc][y0 * N * W + x0 - x]->setPalette(Pal);
			}
		}
		...
		if (neighbor(i, 3) != -1) {
			...
			router_is_active[noc][i] += probes_queue[noc][i][3];
			for (int x = 1; x < N - 1; x++) {
				QPalette Pal(palette()); Pal.setColor(QPalette::Background, col);
				wid_map[noc][y0 * N * W + x0 + x]->setPalette(Pal);
				wid_map1[noc][y0 * N * W + x0 + x]->setPalette(Pal);
	...
	// Routers (draw 2x2 center block like before)
	for (int i = 0; i < TILES_NUM; i++) {
		const Rect &g = tile_rect(i);
		int y0 = N * g.r + 2;
		int y1 = N * g.r + 3;
		...
		QPalette Pal0(palette()), Pal1(palette()), Pal2(palette()), Pal3(palette());
		QColor col = (noc % NOCS_NUM == 0) ? QColor("#bfbfbf") : QColor("#7f7f7f");
		...
		wid_map[noc][y0 * N * W + x0]->setPalette(Pal0);
		wid_map[noc][y0 * N * W + x1]->setPalette(Pal1);
		wid_map[noc][y1 * N * W + x0]->setPalette(Pal2);
		wid_map[noc][y1 * N * W + x1]->setPalette(Pal3);
	}
}
#endif
...
void EspMonMain::get_statistics()
{
    ...
    int missing_dir_ports = 0; // number of absent N/S/W/E ports
    for (int k = 0; k < TILES_NUM; ++k)
      for (int d = 0; d < 4; ++d)
        if (neighbor(k, d) == -1) missing_dir_ports++;
    
    float denom = float(NOCS_NUM * TILES_NUM * DIRECTIONS) - float(NOCS_NUM * missing_dir_ports);
    // protect against divide-by-zero if someone misconfigures tiles[]
    if (denom <= 0.0f) denom = 1.0f;
    
    global_ave_bandwidth /= denom;
